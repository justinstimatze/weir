...
- A first macOS audit pass, done without a Mac, against FreeBSD's own man pages as the closest reachable proxy for Apple's BSD userland. `grep -m`/`-r` and `find -exec ... {} +` all check out, so `grep-head-trim`, `find-exec-semi`, and the three `rg-*-misfire` rules need no changes there. `ps`/`awk` weren't checked at all yet — lower priority, since no existing rule parses either tool's flags directly.
