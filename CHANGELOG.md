...
- **macOS coverage.** `release.yml` gained a `macos-check` job, gated on the same `v*` tag `goreleaser` already builds `darwin/amd64`+`arm64` binaries from — the binaries existed before anything ever ran on the platform they target. Runs `go vet`/`test -race`/`build`/the install smoke test on `macos-latest`, plus a step dumping real `grep`/`sed`/`find`/`ps`/`awk` `--help`/`man` text to the CI log for auditing `rules.go`'s classic-tool assumptions once a tag actually triggers it. Deliberately not on the `ci.yml` push/PR path — `macos-latest` bills at roughly 10x the Linux per-minute rate on a private repo.
- A first macOS audit pass, done without a Mac, against FreeBSD's own man pages as the closest reachable proxy for Apple's BSD userland. `grep -m`/`-r` and `find -exec ... {} +` all check out, so `grep-head-trim`, `find-exec-semi`, and the three `rg-*-misfire` rules need no changes there. One real gap surfaced and stayed unaddressed on purpose: BSD `sed -i` takes a mandatory argument glued to the flag where GNU's is optional, the same shape of landmine `sd-in-place-write` already covers for the other tool. No rule got written for it — `rules.go` has zero OS-detection anywhere, and a rule here would misfire on every Linux install. `ps`/`awk` weren't checked at all yet; lower priority, since no existing rule parses either tool's flags directly.
