...
- **Regex-based rule matching does not parse shell quoting for advisory rules.** A pipe inside a quoted string (e.g. `grep "a|b" file`) can trigger any *advisory* rule that looks for an unquoted pipe. Advisory false positives cost only one extra context line, so this is accepted. **Block-mode rules** additionally suppress matches that land inside a single- or double-quoted string (see `internal/suggest/quotes.go`) — so `git commit -m "...which..."` and similar commit-body prose will not be refused. The quote check is an approximation: it tracks `'` and `"` with `\`-escape handling, but does not separately model heredoc bodies, `$'...'`, or backticks. If a block-mode false positive still slips through, bypass with `WEIR_SUGGEST_SKIP=1`.
